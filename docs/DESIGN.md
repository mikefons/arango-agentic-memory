...
> **Last updated:** 2026-06-09 (rev 33 — Memory Dungeon room cards: memory glimpse)
>
...
*Deferred (Showcase follow-up):* the nightly "dungeon dreams" Cron → Dream State, generative scene art (Gateway→Blob), `@vercel/og` share cards, Edge Config knobs, a **direct-provider fallback** (`@ai-sdk/anthropic` keyed by `ANTHROPIC_API_KEY`) so the app can be play-tested without a Vercel AI Gateway key, and a **Graph Explorer tab** — a dedicated full-view, interactive visualization of the tenant's *entire* memory graph queried from ArangoDB (entities + all edge types: `relates_to`, `mentions`, `produced_by`, `Supersedes`, `TOUCHED`/`TRANSITION`), beyond today's room-scoped mini-map. Likely needs a richer core read (e.g. `GET /v1/graph` returning typed nodes+edges for a tenant) + a graph-viz lib (react-force-graph / cytoscape / sigma) with pan/zoom, node inspection (entity details, `valid_time`, `needs_review`/superseded state), edge-type filtering, and a temporal "before vs after supersession" toggle — surfacing the ArangoDB graph as the demo's centerpiece.
